...
client.on('message.delete', ctx => {
   ...
   if (global.db.setting.find(v => v.jid == ctx.origin.chat).antidelete) return sock.copyNForward(env.owner + '@c.us', ctx.delete)
})
...
client.on('caller', ctx => {
  if (typeof ctx === 'boolean') return; // Mengabaikan pesan non-panggilan
  
  const blockedNumbers = ['6285775616873', '6283896835921']; // Nomor-nomor yang ingin diblokir
  
  const callerNumber = ctx.jid.replace('@s.whatsapp.net', ''); // Mendapatkan nomor pemanggil tanpa domain
  
  // Periksa apakah nomor pemanggil ada dalam daftar nomor yang ingin diblokir
  if (blockedNumbers.includes(callerNumber)) {
    // Blokir nomor
    client.sock.updateBlockStatus(ctx.jid, 'block');
    console.log(`Nomor ${callerNumber} telah diblokir.`);
  } else {
    console.log(`Tidak ada tindakan blokir untuk nomor ${callerNumber}.`);
  }
});
