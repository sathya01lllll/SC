...
exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (command === 'ytmp4' || command === 'ytv' || command === 'ytdl' || command === 'youtubedl') {
            if (!args || !args[0]) return client.reply(m.chat, Func.example(isPrefix, command, 'https://youtu.be/zaRFmdtLhQ8'), m)
            if (!/^(?:https?:\/\/)?(?:www\.|m\.|music\.)?youtu\.?be(?:\.com)?\/?.*(?:watch|embed)?(?:.*v=|v\/|\/)([\w\-_]+)\&?/.test(args[0])) return client.reply(m.chat, global.status.invalid, m)
            client.sendReact(m.chat, '🕒', m.key)
         ...
         return client.reply(m.chat, '🚩 *Maaf terjadi Error, Harap lapor ke Owner*', m)
      }
   },
   error: false,
   owner: true,
